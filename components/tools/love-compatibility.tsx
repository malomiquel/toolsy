...
const selectKeys = [
  "personality", "schedule", "food", "weekend",
  "music", "animal", "love", "tidiness", "money", "communication", "travel",
] as const;
...
const zodiacCompat: Record<string, Record<string, number>> = {
  "Bélier":     { "Bélier": 10, "Taureau": 2,  "Gémeaux": 12, "Cancer": 0,  "Lion": 15, "Vierge": 2,  "Balance": 8,  "Scorpion": 3, "Sagittaire": 15, "Capricorne": 0,  "Verseau": 7,  "Poissons": 3 },
  "Taureau":    { "Taureau": 10, "Gémeaux": 2,  "Cancer": 12, "Lion": 3,  "Vierge": 15, "Balance": 2,  "Scorpion": 8,  "Sagittaire": 0, "Capricorne": 15, "Verseau": 0,  "Poissons": 7 },
  "Gémeaux":    { "Gémeaux": 10, "Cancer": 2,  "Lion": 7,  "Vierge": 3,  "Balance": 15, "Scorpion": 0,  "Sagittaire": 8,  "Capricorne": 0, "Verseau": 15, "Poissons": 2 },
  "Cancer":     { "Cancer": 10, "Lion": 3,  "Vierge": 7,  "Balance": 0,  "Scorpion": 15, "Sagittaire": 0, "Capricorne": 8,  "Verseau": 2,  "Poissons": 15 },
  "Lion":       { "Lion": 10, "Vierge": 2,  "Balance": 12, "Scorpion": 3,  "Sagittaire": 15, "Capricorne": 0, "Verseau": 7,  "Poissons": 2 },
  "Vierge":     { "Vierge": 10, "Balance": 2,  "Scorpion": 12, "Sagittaire": 2,  "Capricorne": 15, "Verseau": 0,  "Poissons": 7 },
  "Balance":    { "Balance": 10, "Scorpion": 3,  "Sagittaire": 7,  "Capricorne": 2,  "Verseau": 15, "Poissons": 2 },
  "Scorpion":   { "Scorpion": 10, "Sagittaire": 2,  "Capricorne": 7,  "Verseau": 0,  "Poissons": 15 },
...
const questions: { key: SelectKey; label: string; options: { value: string; label: string }[] }[] = [
  {
  ...
  {
    key: "schedule", label: "Rythme de vie",
    options: [
  ...
  {
    key: "money", label: "Rapport à l'argent",
    options: [
  ...
  {
    key: "food", label: "Bouffe préférée",
    options: [
  ...
  {
    key: "weekend", label: "Week-end idéal",
    options: [
  ...
  {
    key: "animal", label: "Animal préféré",
    options: [
  ...
  {
    key: "love", label: "Langage amoureux",
    options: [
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      >
        ...
        <CalendarIcon className={`size-3.5 shrink-0 ${zodiac ? colorClass : "text-muted-foreground"}`} />
      </PopoverTrigger>
...
export function LoveCompatibility() {
  ...
  const update1 = (key: string, value: string | Date | undefined) =>
    setPerson1((p) => ({ ...p, [key]: value }));
  const update2 = (key: string, value: string | Date | undefined) =>
  ...
  const calculateCompatibility = () => {
    ...
    const common = new Set(n1.split("").filter((l) => l.trim() && n2.includes(l)));
  ...
  const getResultMessage = (s: number) => {
    ...
    if (s >= 45) return "Compatibilité moyenne, mais l'amour fait des miracles !";
  ...
  const isFormValid =
    person1.name && person2.name && person1.birthdate && person2.birthdate &&
        ...
        <CardContent className="p-5 space-y-2.5">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="flex items-center justify-center gap-1.5 pb-0.5">
              ...
              <Label className="text-xs font-medium text-pink-600 uppercase tracking-wide">Personne 1</Label>
            </div>
            <div className="flex items-center justify-center gap-1.5 pb-0.5">
              <Heart className="size-3.5 text-purple-500 fill-purple-500" />
              <Label className="text-xs font-medium text-purple-600 uppercase tracking-wide">Personne 2</Label>
            </div>
          </div>

          {/* Prénom */}
          <div className="grid grid-cols-2 gap-2">
            <Input value={person1.name as string} onChange={(e) => update1("name", e.target.value)} placeholder="Prénom" />
            <Input value={person2.name as string} onChange={(e) => update2("name", e.target.value)} placeholder="Prénom" />
          </div>

          {/* Date de naissance */}
          <div className="grid grid-cols-2 gap-2">
            <DatePickerButton value={person1.birthdate as Date | undefined} onChange={(d) => update1("birthdate", d)} zodiac={zodiac1} color="pink" />
            <DatePickerButton value={person2.birthdate as Date | undefined} onChange={(d) => update2("birthdate", d)} zodiac={zodiac2} color="purple" />
          </div>

          {/* Questions */}
          {questions.map((q) => (
            <div key={q.key} className="grid grid-cols-2 gap-2">
              <Select value={person1[q.key] as string} onValueChange={(v) => update1(q.key, v)}>
                <SelectTrigger className="w-full">
                  {person1[q.key]
                    ? <span className="truncate">{getLabel(q.key, person1[q.key] as string)}</span>
              ...
              <Select value={person2[q.key] as string} onValueChange={(v) => update2(q.key, v)}>
                <SelectTrigger className="w-full">
                  {person2[q.key]
                    ? <span className="truncate">{getLabel(q.key, person2[q.key] as string)}</span>
                    : <span className="text-muted-foreground truncate">{q.label}</span>
                  }
                </SelectTrigger>
                <SelectContent>
          ...
          {/* Bouton */}
          <Button onClick={calculateCompatibility} disabled={!allValid} className="w-full h-9 gap-2">
            ...
            <CardContent className="p-6 flex flex-col items-center justify-center h-full gap-5">
              ...
              >
                <div className={`text-6xl font-bold tabular-nums ${
                  result >= 60 ? "text-pink-600" : result >= 30 ? "text-orange-500" : "text-blue-500"
                }`}>
                  <AnimatedScore target={result} />%
                </div>
                <div className="text-xs text-neutral-400 mt-1">de compatibilité</div>
              </div>
                ...
                {Array.from({ length: 5 }).map((_, i) => {
                  ...
                  return (
                    <Heart
                      ...
                      style={{ transitionDelay: `${800 + i * 100}ms`, transitionDuration: "400ms" }}
                    />
                  );
                })}
              </div>
